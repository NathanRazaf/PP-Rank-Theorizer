...
export const updateProfileWithScores = async ({
    ...
    try {
        ...
        return handleUserUpdateResponse(data);
    } catch (error) {
        console.log(error);
...
export function createUserUpdateBodyRequest({ profile, scores }: { profile: User; scores: Score[] }){
    return {
        profile: {
...
export function handleUserUpdateResponse(data: GetFullUserResponse): {
    profile: User;
    scores: Score[];
} {
    return {
